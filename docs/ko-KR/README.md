...
| `sources/` | 나중에 검증할 근거 | 추가만 하고 본문은 고치지 않습니다 | 스키마 |
...
`sources/`에는 원문 그대로(`type: source-raw`)와 정제본(`type: source-summary`)이 함께 들어갑니다. 잘 쓴 문서일 필요가 없습니다. 승급된 `context/`의 주장을 나중에 검증하기 위해 있는 자리입니다.
...
스물아홉 개이고 다섯 갈래로 나뉩니다. 넣고, 올리고, 조회하고, 다시 꺼내고, 관리합니다.
...
| `status` | 현황과 밀린 것, 다음에 할 일을 보여 줍니다 |
...
| `sync` | git 이력에서 날짜 필드를 바로잡습니다. 이력이 없으면 파일명 접두사를 봅니다. `--dry-run`이 기본입니다 |
| `rules show` | 이 위키에 적용되는 규칙 전부를 읽기 전용으로 보여 줍니다 |
| `model` | 임베딩 모델을 내려받고 상태를 봅니다 |
...
커맨드 출력은 기본이 한국어입니다. 플래그나 환경변수로 바꿉니다.
...
허용값은 `ko`와 `en`입니다. 이 설정은 `engram.yaml`에 두지 않습니다. 그 파일은 커밋되어 팀이 공유하는데 출력 언어는 읽는 사람의 것이기 때문입니다. 근거는 [ADR 0049](../decisions/0049-cli-output-language.md)에 있습니다.

## 어디까지 왔나요

0.1부터 1.0까지 끝났습니다. 위 커맨드 스물아홉 개는 전부 동작합니다. 첫 릴리스는 저장소 공개와 함께 나갑니다.
