...
실습 재료를 자기 위키에 집어넣습니다. 무엇을 남기고 무엇을 버릴지 정하는 것이 이 단계의 전부입니다.
...
**통째로 넣지 않습니다.** 읽고 남길 것을 직접 고릅니다. 아래는 하나의 예이며 정답이 아닙니다.

```
engram capture --title "응답 지연 회의" "새벽 2시부터 p99 320ms, 40분 뒤 회복. 배포 없었고 트래픽도 평소보다 적었다. 정산 배치가 같은 디비를 본다는 가설이 나왔으나 왜 이날만인지는 미확인. 캐시 도입은 무효화 문제로 보류. 확인할 것: 지난달 같은 날 이력, 디비 대기 지표, 배치 소요 시간의 데이터량 의존성."
```

```
inbox에 넣었습니다: /Users/사용자/engram-wiki/inbox/2026-08-17-응답-지연-회의.md
...
강사가 물을 것 하나. **"결정된 것과 아직 모르는 것을 갈랐습니까."** 이 회의에서 결정된 것은 "캐시 보류"와 "각자 확인할 것 셋"이고, 나머지는 전부 가설입니다. 그 구분이 4단계 승급의 재료가 됩니다.
...
하나로 넣을지 셋으로 나눌지가 수강생의 판단입니다. **나누기를 권합니다.** 나중에 검색으로 꺼낼 때 한 덩어리에 세 주제가 섞여 있으면 어느 것으로도 잘 안 걸립니다.

```
engram capture --title "배포 실패: 설정 키 누락" "운영에만 없는 설정 키를 읽어 헬스체크 실패, 롤백. 로컬과 스테이징에는 있어서 사전에 못 봤다."
engram capture --title "필수 설정 키를 기동 때 검사" "없으면 빈 값으로 넘어가 나중에 이상하게 터진다. 반만 동작하느니 안 뜨는 편이 낫다는 결론."
engram capture --title "기록이 안 쌓이는 이유" "터진 직후엔 수습하느라 못 적고 끝나면 잊는다. 제대로 적어야 한다는 부담이 아예 안 적게 만든다."
```
...
지금 위키에 있는 것과 이어지는 것 하나만 남깁니다.

```
engram capture --title "캐시를 걷어낸 사례" "무효화 버그를 쫓는 시간이 캐시로 아낀 시간을 넘어섰다는 이유. 도입 사례와 함께 볼 것."
```

다 넣으면 `inbox`가 쓰레기가 됩니다. **넣지 않기로 하는 것도 이 단계의 실습입니다.**
...
```
검사한 파일 7개, error 0, warn 5, reject 0
```

**`warn` 다섯이 정상입니다.** 전부 `graph.orphan`이며 방금 넣은 `inbox` 문서가 아무와도 이어져 있지 않다는 뜻입니다. 아직 이을 곳이 없으니 당연합니다. **`error`와 `reject`가 0인지만 봅니다.**

```
밀린 것 (기준 2026-08-17)
  inbox 문서 5개, 전부 오늘 들어왔습니다
  지금 승급할 수 있는 문서 0개

다음 행동
  - 링크 채우기
    inbox 문서 5개가 있지만 지금 올릴 수 있는 것이 없습니다. 승급하려면
...
| 예시 답 | 본문의 `capture` 내용은 예이며 정답이 아니라고 명시합니다 | 정답을 주면 베낍니다 |
