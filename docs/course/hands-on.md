...
`주간뉴스.txt`는 읽을거리 다섯과 도구 소식과 짧은 소식이 들어 있습니다. **대부분 버립니다.**

**여기서 하는 판단은 넣느냐 마느냐 하나입니다.** 뉴스레터 전체를 넣을 이유는 없습니다. 지금 위키에 있는 것과 이어지는 항목 하나만 골라, 그 문단을 그대로 옮겨 넣습니다.

```
engram capture --title "캐시를 걷어낸 사례" "캐시를 넣고 2년 뒤 걷어낸 팀. 원본 질의가 빨라져서가 아니라 무효화 버그를 쫓는 시간이 캐시로 아낀 시간을 넘어섰기 때문이라고. 앞의 캐시 도입 사례들과 함께 읽으면 좋습니다."
```

원문 문단이라 짧아서 인자로 줍니다. **요약이 아니라 발췌입니다.** 다 넣으면 `inbox`가 쓰레기가 됩니다. **넣지 않기로 하는 것이 이 단계의 유일한 판단입니다.**
...
```
검사한 파일 5개, error 0, warn 3, reject 0
```

**`warn` 셋이 정상입니다.** 전부 `graph.orphan`이며 방금 넣은 `inbox` 문서가 아무와도 이어져 있지 않다는 뜻입니다. 아직 이을 곳이 없으니 당연합니다. **`error`와 `reject`가 0인지만 봅니다.**

```
밀린 것 (기준 2026-08-17)
  inbox 문서 3개, 전부 오늘 들어왔습니다
  지금 승급할 수 있는 문서 0개

다음 행동
  - 링크 채우기
    inbox 문서 3개가 있지만 지금 올릴 수 있는 것이 없습니다. 승급하려면
...
- 50분입니다. 커맨드를 치는 시간보다 재료를 읽는 시간이 길어야 합니다.
- **실습 1에서 각자 넣은 내용을 두셋 낭독시킵니다.** 같은 전사문에서 서로 다른 것을 남깁니다. 그 차이가 이 도구가 사람에게 남긴 판단의 크기입니다.
...
- 뉴스레터를 여럿 넣은 수강생도 그대로 둡니다. 4단계에서 승급 대상을 고를 때 부담으로 돌아옵니다.
