...
| `주간뉴스.txt` | 뉴스레터 한 통. **3단계에서 쓰지 않습니다.** 골라서 옮기는 일은 7단계에서 에이전트가 합니다 |
...
심는 문장은 이것입니다. **거절 사유는 하나뿐입니다. 거절 경험이 곧 교육입니다.**
...
**막힌 이유가 하나입니다.** 길이도, 형식도, 주제도 묻지 않았습니다. 이 문서가 위키의 다른 무엇과도 이어져 있지 않다는 것만 막았습니다.
...
**메시지 마지막 줄은 지금 따르지 않습니다.** `--related`를 두 번 줘서 그 자리에서 통과시킬 수 있지만, 그러면 68줄 원문이 정리 없이 그대로 `context`에 올라갑니다. 관문에서 정리하는 것이 이 단계의 요점입니다.

## 관문에서 정리합니다

3단계에서 미룬 일이 여기 있습니다. **68줄 전사 원문을 읽고 남을 것을 씁니다.**

### 새 본문 파일을 만듭니다

**이 파일은 직접 만듭니다.** engram이 만들어 주지 않습니다. 위키 밖에 두는 임시 파일이고 이름은 아무거나 됩니다.

```
vi ~/정리한본문.md
```

**프론트매터(`---` 블록)는 적지 않습니다. 본문만 적습니다.** 프론트매터는 `inbox` 문서에 이미 있고 그대로 남습니다.

무엇을 쓸지는 각자 다릅니다. **정답 본문을 주지 않습니다.** 아래 둘만 지킵니다.

- **결정된 것**과 **아직 모르는 것**을 나눕니다. 전사문에서 결정된 것은 "캐시 보류"와 "확인할 것 셋"뿐이고 나머지는 전부 가설입니다.
- **다른 문서로 잇습니다.** 본문에 `[[캐시-무효화-정리]]`를 씁니다. 아직 없는 문서입니다. 바로 다음에 만듭니다.
...
- 캐시 도입은 보류한다. 원인을 모르는 상태에서 얹으면 증상만 가린다.
- 각자 확인할 것 셋을 나눴다.

## 아직 모르는 것

- 지연이 특정 엔드포인트에 몰리는지 전체에 걸치는지
- 커넥션 풀 고갈인지 쿼리 자체인지

## 이어지는 것

캐시를 검토하려면 무효화 설계를 먼저 봐야 한다. [[캐시-무효화-정리]]에 정리했다.
```

### 통째로 갈아 끼웁니다

```
engram update inbox/2026-08-17-응답-지연-회의-전사.md --body-from ~/정리한본문.md
```

```
갱신했습니다: inbox/2026-08-17-응답-지연-회의-전사.md
본문 교체: /Users/사용자/정리한본문.md
updated에 갱신 날짜 2026-08-17를 기록했습니다
```

**`update`가 하는 일은 하나입니다. 프론트매터는 그대로 두고 본문만 이 파일 내용으로 통째로 바꿉니다.** 원문 68줄이 방금 쓴 내용으로 교체됩니다. 문서를 열어 확인합니다.

**원문은 여기서 사라집니다.** `inbox`는 임시 계층입니다. 원문 자체를 남겨야 한다면 3단계에서 `source`로 넣었어야 합니다.
...
**링크 2개**입니다. `--related index` 하나와 본문의 `[[캐시-무효화-정리]]` 하나입니다. 게이트는 문서가 나가는 링크만 셉니다.
...
앞 단계에서 쓴 `[[캐시-무효화-정리]]`가 이제 실제 문서를 가리킵니다.

## index.md는 저절로 안 바뀝니다

여기서 대부분 한 번 묻습니다. **문서를 셋이나 올렸는데 `index.md`가 그대로입니다.**

`--related index`는 **새 문서에서 색인으로 나가는** 링크입니다. 반대 방향은 만들지 않습니다. **engram은 `index.md`를 자동으로 고치지 않습니다.** `init`이 만들고, 그 뒤로는 사람이 `update`로 씁니다.
...
무엇을 색인 앞에 둘지는 판단이고 engram은 판단하지 않습니다. 손으로 씁니다.

```
engram update index.md --body-from ~/색인본문.md
```

## 혼자 해 봅니다

`inbox`에 슬랙 문서가 하나 남아 있습니다. 같은 순서로 올립니다.

1. 본문을 정리해서 `update --body-from`
2. `promote --type incident --slug 설정-키-누락-사건 --related index`

**본문에 `[[응답-지연-사건]]`을 넣습니다.** 증상이 비슷해서 헷갈리기 쉬운 두 사건이고, 그 사실을 적어 두는 것이 나중에 도움이 됩니다.
...
**고아 문서가 2개에서 0개가 되었습니다.** 3단계 끝에서 둘이었습니다. 게이트가 그것을 강제한 결과입니다.

```
git add -A && git commit -m "4단계: 승급, 이름 변경, 보관"
```

## 강사 노트

- 50분입니다. 첫 거절을 다 같이 겪는 것으로 시작합니다.
- **거절 메시지를 소리 내어 읽힙니다.** 무엇이 막혔는지, 무엇은 안 막혔는지 둘 다 말하게 합니다. 길이와 형식과 주제를 안 막았다는 것이 요점입니다.
- 본문 정리에 20분을 줍니다. 여기가 이 과정 전체에서 사람이 가장 오래 생각하는 자리입니다.
- `ls inbox`와 `ls sources`를 승급 직후에 반드시 시킵니다. 이동과 파생의 차이는 말로 설명하는 것보다 없어진 파일을 보는 것이 빠릅니다.
- **`index.md`를 안 고치냐는 질문이 반드시 나옵니다.** 나오면 반갑게 받습니다. 고아 판정과 맞물린 자리이고 5단계 `backlinks`로 이어집니다.
- `min_wikilinks`를 1로 낮춰 보고 싶다는 수강생이 나오면 `engram.yaml`에서 바꾸게 둡니다. 8단계에서 규칙이 자기 것이 된다는 이야기의 예고편입니다.

## 이 단계에서 확정한 것

| 결정 | 내용 | 이유 |
|---|---|---|
| 거절을 먼저 겪습니다 | 정리 전에 그대로 `promote`해서 막힙니다 | 거절 경험이 교육입니다. 성공부터 보여 주면 게이트가 왜 있는지 안 남습니다 |
| 정리는 여기서 합니다 | `update --body-from`으로 본문을 갈아 끼운 뒤 승급합니다 | 3단계에서 미룬 판단이 이 자리입니다. 관문은 한 번입니다 |
| 정답 본문을 주지 않습니다 | 골격 둘만 제시합니다 | 각자 다르게 쓰는 것이 정상이고 그 차이가 판단의 크기입니다 |
| `inbox` 원문이 사라지는 것을 짚습니다 | `ls inbox`로 직접 보게 합니다 | 보존이 필요하면 3단계에서 `source`였어야 한다는 것을 몸으로 알게 합니다 |
| `index.md`는 자동 갱신하지 않습니다 | `backlinks index`로 대신 봅니다 | 색인이 전부를 가리키면 고아 판정이 영원히 0이 됩니다 |
| 슬랙 문서는 혼자 올립니다 | 순서만 주고 본문은 각자 씁니다 | 두 번째는 스스로 해야 손에 남습니다 |
| `mv`는 `--dry-run` 먼저 | 두 번 칩니다 | 링크를 따라가 여러 파일을 고치는 커맨드입니다 |
