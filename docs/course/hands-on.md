...
**이것을 손으로 푸는 것은 이 도구의 사용법이 아닙니다.** 68줄을 읽고, 남길 것을 정하고, 이을 문서를 찾고, 슬러그를 짓는 일입니다. 그 일을 사람이 하면 3단계에서 없앤 관문이 여기서 되살아납니다. **여기서 에이전트를 붙입니다.**

## 에이전트를 붙입니다

**먼저 어디에 심을지 봅니다.** 이 커맨드는 홈을 건드리므로 확인하고 칩니다.
...
**홈에 실제로 있는 스킬 디렉토리를 찾아 전부에 심습니다.** 없는 도구를 위해 디렉토리를 만들지 않습니다. 하나도 못 찾으면 실패하므로 그때는 `--dir`로 직접 줍니다.

### 전역이냐 이 위키만이냐

기본값은 **전역**입니다. 홈에 심으므로 **어느 디렉토리에서 에이전트를 띄우든 이 스킬이 잡힙니다.**
...
전역이 기본값인 이유는 **스킬 문서가 위키와 무관하기 때문**입니다. 위키의 임계값이나 허용값을 담지 않습니다. 그래서 위키가 몇 개든 하나면 되고, 위키 디렉토리 밖에서 에이전트를 띄웠을 때도 필요합니다. 회의 노트가 있는 다른 폴더에서 "이거 위키에 넣어 줘"가 되어야 합니다.

**시험 중이라면 격리합니다.** 아직 검증 중인 스킬이 모든 세션에 얹히는 것을 원치 않을 때입니다.

```
mkdir -p ~/engram-wiki/.claude/skills
engram skills install --dir ~/engram-wiki/.claude/skills
```

`--dir`은 **스킬 루트**를 받습니다. 그 아래 `engram/SKILL.md`를 만듭니다. 이미 있는 디렉토리여야 합니다.
...
이미 있는 파일은 덮지 않습니다. 충돌하면 무엇이 충돌하는지 알리고 멈춥니다. 덮으려면 `--force`를 줍니다.

**심는 문서는 정적입니다.** 위키의 임계값이나 허용값을 담지 않습니다. 그 위키의 규칙은 에이전트가 `engram rules show`로 직접 얻습니다. 그래서 한 번 심으면 위키가 몇 개든 통합니다.

심은 문서를 열어서 읽습니다. 짧습니다.

```
cat ~/.claude/skills/engram/SKILL.md
```

**여기가 이 과정에서 제일 중요한 페이지입니다.** 에이전트가 무엇을 할 수 있고 무엇을 못 하는지가 전부 이 문서에 있습니다.
...
검색이 먼저 도는 편이 낫습니다. 색인을 한 번 만듭니다.
...
문서 4개, 토큰 1038개, 크기 44610 바이트
```

이제 에이전트에게 말합니다. **커맨드를 알려 주지 않습니다.**

```
~/engram-wiki 의 inbox 에 있는 응답 지연 회의 전사를 승급할 수 있게 준비해 줘
```

에이전트가 셋을 합니다. 스킬 문서가 그 순서를 지시합니다.

**1. 이을 곳을 찾습니다.** 사람에게 묻지 않고 검색합니다.
...
{
  "results": [
    { "rank": 1, "slug": "2026-02-18-캐시-무효화를-설계하는-네-가지-방법",
      "score": 3.28, "path": "sources/2026-02-18-캐시-무효화를-설계하는-네-가지-방법.md" }
  ]
}
```

**이을 곳은 `context` 문서 중에서 고릅니다.** 검색 결과의 `path`가 `inbox/`로 시작하는 것은 쓰지 않습니다. `inbox` 슬러그에는 날짜 접두사가 붙어 있는데 그 문서가 승급되면 접두사가 떨어져 **링크가 깨집니다.** `mv`와 달리 `promote`는 들어오는 링크를 안 고쳐 줍니다. 실제로 확인했습니다.

```
깨진 위키링크: [[2026-08-17-배포-실패-스레드]]에 해당하는 문서가 없습니다
```

**2. 본문을 정리해 갈아 끼웁니다.** 원문을 읽고 남을 것만 씁니다. 결정된 것과 아직 모르는 것을 나누고, 1번에서 찾은 문서로 `[[슬러그]]`를 심습니다.
...
```
갱신했습니다: inbox/2026-08-17-응답-지연-회의-전사.md
본문 교체: /tmp/...
updated에 갱신 날짜 2026-08-17를 기록했습니다
```

**원문 자리에 갈아 끼웁니다. 정리본을 별도 문서로 하나 더 만들지 않습니다.** 그러면 원문과 정리본이 둘 다 `inbox`에 남아 적체됩니다. 원문을 오래 둘 값어치가 있으면 에이전트가 갈아 끼우기 전에 `source`를 먼저 제안합니다.

**3. 실행 가능한 한 줄로 제안하고 멈춥니다.**

```
정리했습니다. 68줄 전사문에서 결정된 것은 "캐시 보류"와 "확인할 것 셋"뿐이고
나머지는 가설이라 그 둘을 갈라 적었습니다. 캐시 얘기가 나와서
sources 의 무효화 문서를 [[캐시-무효화-정리]] 로 이어 뒀습니다.
그 문서도 함께 올려야 링크가 붙습니다.
...
**바뀐 본문을 먼저 읽습니다.** 이것이 이 단계에서 사람이 하는 유일한 일입니다.
...
**링크 2개**입니다. `--related index` 하나와 에이전트가 본문에 심은 `[[캐시-무효화-정리]]` 하나입니다. 게이트는 문서가 나가는 링크만 셉니다.
...
앞 단계에서 심은 `[[캐시-무효화-정리]]`가 이제 실제 문서를 가리킵니다.

## 왜 둘을 함께 올렸나

**위키가 비어 있으면 이을 곳이 없습니다.** 4단계를 시작할 때 `context`에 문서가 하나도 없었습니다. 게이트는 링크 2개를 요구하는데 가리킬 대상이 `index`뿐이었습니다.

그래서 첫 승급은 **문서 하나가 아니라 서로 잇는 묶음으로** 올라갑니다. 에이전트가 그것을 알아서 제안합니다. 스킬 문서가 그렇게 지시합니다.

```
위키가 비어 있어 이을 곳이 없으면 서로 잇는 문서 둘을 함께 제안한다.
```

위키가 자라면 이 문제는 사라집니다. 이을 곳이 생기기 때문입니다.

## index.md는 저절로 안 바뀝니다
...
**자동으로 채우지 않는 이유가 있습니다.** `index.md`가 모든 문서를 가리키면 **고아 판정이 영원히 0이 됩니다.** 색인에서 들어오는 링크도 링크로 세기 때문입니다. 직접 확인한 결과입니다.
...
`inbox`에 슬랙 문서가 하나 남아 있습니다. 같은 방식으로 올립니다.

```
inbox 에 남은 배포 실패 스레드도 승급할 수 있게 준비해 줘
```

**커맨드를 치지 않습니다.** 에이전트가 검색하고, 정리하고, 제안합니다. 사람은 본문을 읽고 `promote`를 실행합니다.
...
올리고 나서 이름이 마음에 안 들 수 있습니다. `mv`가 링크를 따라가 전부 고칩니다.
...
고친 링크 2건:
  context/설정-키-누락-사건.md: 1건
  context/캐시-무효화-정리.md: 1건
다음: engram lint로 링크 무결성을 확인하세요
```

**두 건이 성격이 다릅니다.** 하나는 본문 위키링크이고 하나는 프론트매터 `related`입니다. 둘 다 고칩니다. 코드 블록 안의 링크 문법은 건드리지 않습니다.

**이 커맨드는 에이전트에게 주지 않습니다.** `mv`는 여러 파일을 한 번에 고칩니다. 스킬 문서가 단계 이동 커맨드를 사람의 것으로 남겨 둔 이유입니다.

## 폐기하기

수명이 끝난 문서는 지우지 않고 보관합니다.
...
**슬러그가 그대로입니다.** 폐기된 사건을 가리키던 링크가 깨지지 않습니다. 지웠다면 다른 문서에 구멍이 생깁니다. 이력이 남는 것이 요점입니다.
...
되짚습니다. 커맨드를 몇 개 쳤는지가 아니라 **무엇을 판단했는지**를 봅니다.
...
- **거절 메시지를 소리 내어 읽힙니다.** 무엇이 막혔는지, 무엇은 안 막혔는지 둘 다 말하게 합니다. 길이와 형식과 주제를 안 막았다는 것이 요점입니다.
- **거절 직후에 "이걸 손으로 풀어 보겠느냐"고 묻습니다.** 68줄을 읽고 정리하겠다는 사람은 없습니다. 그 침묵이 에이전트를 붙이는 이유입니다.
- `SKILL.md`를 화면에 띄우고 쓰기 경계 문단을 함께 읽습니다. **이 과정에서 제일 중요한 페이지입니다.**
- **에이전트가 만든 정리본을 반드시 읽게 합니다.** 그냥 `promote`부터 치는 수강생이 나옵니다. 그러면 이 도구가 승급 게이트를 둔 의미가 없습니다.
...
| 결정 | 내용 | 이유 |
|---|---|---|
| 에이전트를 4단계에서 붙입니다 | 7단계가 아니라 여기입니다 | 4단계가 처음으로 사람이 글을 써야 하는 자리입니다. 거기에 에이전트가 없으면 실습이 도구의 논지와 반대가 됩니다 |
| 거절을 먼저 겪습니다 | 정리 전에 그대로 `promote`해서 막힙니다 | 거절 경험이 교육입니다. 그리고 에이전트를 왜 붙이는지가 그 자리에서 설명됩니다 |
| 정리는 에이전트가 합니다 | 사람이 본문을 쓰지 않습니다 | 68줄을 읽고 정리하는 일이 이 도구가 없애려는 일입니다. 사람에게 시키면 관문이 두 번이 됩니다 |
| 이을 곳도 에이전트가 찾습니다 | `search`로 찾고 사람에게 묻지 않습니다 | 사람에게 묻는 것은 검색을 안 돌렸다는 뜻입니다 ([ADR 0052](../decisions/0052-agent-prepares-the-promotion-and-the-human-decides-it.md)) |
| 사람이 하는 것은 셋입니다 | 값어치 판단, 사실 확인, `promote` 실행 | 승급 결정만 사람의 것입니다. 그래서 MCP에도 `promote`가 없습니다 |
| 첫 승급은 묶음입니다 | 서로 잇는 문서 둘을 함께 올립니다 | 빈 위키에는 이을 곳이 없습니다. 게이트가 링크 2개를 요구합니다 |
...
| 이을 곳은 `context` 문서입니다 | `inbox` 문서를 가리키지 않습니다 | 승급하면 날짜 접두사가 떨어져 링크가 깨지고 `promote`는 그것을 안 고칩니다 |
