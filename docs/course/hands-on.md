...
- **정리는 에이전트가 하고 승급은 사람이 정합니다.** 사람이 문장을 쓰는 자리가 아닙니다.
...
이 단계에서 사람이 터미널에 치는 것은 여섯 번뿐입니다. 나머지는 에이전트가 합니다.
...
에이전트가 셋을 합니다. 아래는 **에이전트가 부르는 것이고 사람은 치지 않습니다.**
...
> **본문을 정리해 갈아 끼웁니다.** 원문을 읽고 남을 것만 씁니다. 본문에 `[[2026-02-18-캐시-무효화를-설계하는-네-가지-방법]]`을 심습니다. `sources`의 원본이고 **슬러그에 날짜 접두사가 붙어 있습니다.**
>
> `engram update inbox/2026-08-17-응답-지연-회의-전사.md --body-from <임시파일> --wiki ~/engram-wiki`
>
> 원문 자리에 갈아 끼웁니다. 정리본을 별도 문서로 하나 더 만들지 않습니다. 그러면 원문과 정리본이 둘 다 `inbox`에 남아 적체됩니다. 원문을 오래 둘 값어치가 있으면 갈아 끼우기 전에 `source`를 먼저 제안합니다.

> **`--dry-run`으로 먼저 검증하고 제안합니다.**
>
...
> ```
>
> **게이트 통과 여부는 짐작하지 않습니다.** 아무것도 쓰지 않으니 몇 번이든 돌릴 수 있고, 거절당하면 `--related`를 고쳐 다시 돌립니다. 통과한 뒤에야 `--dry-run`만 뗀 커맨드를 사람에게 냅니다.

**여기서 멈춥니다. `promote`는 에이전트가 실행하지 않습니다.**

## 4. 사람이 결정합니다

**바뀐 본문을 먼저 읽습니다.** 이것이 이 단계에서 사람이 하는 유일한 판단입니다.

**터미널**

```
cat inbox/2026-08-17-응답-지연-회의-전사.md
```
...
| 볼 것 | 아니면 |
|---|---|
| **올릴 값어치가 있는가** | `context`는 검수된 지식의 자리입니다. 아니면 그냥 둡니다 |
| **틀린 말이 들어갔는가** | 에이전트가 가설을 결론처럼 적기 쉽습니다. 전사문에는 결론이 없습니다 |
| **링크가 맞는가** | 억지로 이은 링크는 나중에 검색을 더럽힙니다 |

**문장을 다시 쓰지 않습니다.** 마음에 안 들면 에이전트에게 무엇이 틀렸는지 말하고 다시 시킵니다. 그게 두 번째 판단입니다.

납득했으면 **제안받은 커맨드를 그대로** 칩니다.

**터미널**

```
engram promote inbox/2026-08-17-응답-지연-회의-전사.md \
  --type incident --slug 응답-지연-사건 --related index
```
...
**커맨드를 치지 않습니다.** 에이전트가 검색하고, 정리하고, 제안합니다. 사람은 본문을 읽고 제안받은 `promote`만 칩니다.
...
| 사람 | 에이전트 |
|---|---|
| 올릴 값어치가 있는지 정했습니다 | 원문을 읽고 정리했습니다 |
| 정리본에 틀린 말이 없는지 봤습니다 | 이을 곳을 검색으로 찾았습니다 |
| `promote`를 실행했습니다 | 실행 가능한 커맨드를 만들어 제안했습니다 |
| 이름을 바꾸고 폐기했습니다 | |

**문장을 쓴 사람이 없습니다.** 그것이 설계입니다. 게이트가 지키는 것은 문장의 품질이 아니라 연결입니다.
...
- **에이전트가 만든 정리본을 반드시 읽게 합니다.** 그냥 `promote`부터 치는 수강생이 나옵니다. 그러면 게이트를 둔 의미가 없습니다.
...
| 사람이 하는 것은 셋입니다 | 값어치 판단, 사실 확인, `promote` 실행 | 승급 결정만 사람의 것입니다. 그래서 MCP에도 `promote`가 없습니다 |
