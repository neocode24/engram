...
>
> 검색 결과에서 `path`가 `context/`로 시작하는 것만 고릅니다. `inbox` 문서를 가리키면 그 문서가 승급될 때 날짜 접두사가 떨어져 링크가 깨지기 때문입니다.

> **본문을 정리해 갈아 끼웁니다.** 원문을 읽고 남을 것만 씁니다.
>
...
그래서 첫 승급은 **문서 하나가 아니라 서로 잇는 묶음으로** 올라갑니다. 에이전트가 알아서 제안합니다. 스킬 문서가 그렇게 지시합니다.
...
| 첫 승급은 묶음입니다 | 서로 잇는 문서 둘을 함께 올립니다 | 빈 위키에는 이을 곳이 없습니다 |
