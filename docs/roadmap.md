...
지금 코드베이스에 TTY 감지도 색도 `NO_COLOR` 처리도 하나도 없다. 모든 줄이 같은 무게라 `error 0, warn 2, reject 0`에서 눈이 무엇을 볼지 고르지 못한다.

**박스 드로잉과 스피너는 넣지 않는다.** 파이프와 CI에서 깨지고, `lint` 출력은 `eject`가 내보낸 Python 린터와 바이트가 같아야 하는 계약이 있으며, 결정론이 명시된 설계 속성이다. 넣을 것은 색과 위계뿐이다.

| 넣을 것 | 근거 |
|---|---|
| TTY 일 때만 색. `NO_COLOR` 존중 | 파이프로 나가면 지금과 바이트가 같아 골든과 동등성 계약이 안 깨진다 |
| 심각도 색 | `reject` 빨강, `warn` 노랑, `ok` 흐리게 |
| 숫자 강조 | 요약 줄에서 수만 굵게 |
| 기본 출력 축소와 `-v` | `status`가 열두 줄인데 대부분 0이다 |

작업 규모는 작다. 색을 붙일 자리가 넷이다. `internal/cli`의 `lint.go` 둘, `doctor.go` 하나, `rules.go` 하나이며 전부 `[error]` 같은 꼬리표다. 카탈로그와 시험은 손대지 않는다. 시험은 `bytes.Buffer`에, 골든은 파이프에 쓰므로 색이 붙지 않는다.

**걸리는 것은 Windows 하나다.** ANSI 이스케이프가 렌더링되려면 `ENABLE_VIRTUAL_TERMINAL_PROCESSING`이 켜져 있어야 하고, 안 켜면 conhost 가 이스케이프를 날것으로 찍어 색이 없는 것보다 나빠진다. `console_windows.go`가 이미 코드페이지를 다루므로([0026](decisions/0026-windows-console-utf8.md)) 같은 자리에서 켜고, 실패하면 색을 끈다. 실기 확인은 Windows 재검증과 한 묶음이다.

### 검색을 다시 본다

1.0의 검색은 토큰 기반 BM25 하나다. [0061](decisions/0061-field-weights-and-what-the-index-is-not.md)이 필드 가중치를 넣어 지금 구조에서 뽑을 수 있는 것을 뽑았고, 남은 것은 구조를 바꿔야 닿는다.

| 남은 것 | 왜 지금 구조로 안 되나 |
|---|---|
| `캐시`와 `cache`가 서로 안 걸린다 | 다른 토큰이다. 시맨틱 층의 몫이다 |
| 라틴 접두 일치 (`config`가 `configuration`을 못 찾음) | 토크나이저가 통짜 토큰을 낸다 |
| 조각 단위 벡터 저장 | 색인이 JSON 파일 하나다 |

**벡터는 `.engram/vectors.bin` 이진 파일에 둔다.** 조각 벡터를 JSON에 담을 이유가 없다. float32 1500 x 1024 가 JSON이면 16.9MB에 디코딩 147ms이고 이진이면 6.1MB에 파싱이 사실상 없다. 색인 본체는 지금대로 JSON에 두고 벡터만 옆에 붙인다.

**SQLite는 지금 근거가 없다.** 실측에서 전체 재색인이 0.1초, 색인 크기가 원문의 1.7배, 조각 1500개 전수 코사인이 1.2ms다. SQLite가 주는 것은 증분 갱신과 ANN 색인인데 둘 다 이 규모에서 필요가 없다. FTS5로 토크나이저를 넘기는 것도 이유가 되지 못한다. **bigram이 이미 조사 문제를 풀고 있다**(0061에 실측이 있다). 재검토 조건은 문서 2000개를 넘거나 전수 코사인이 50ms를 넘을 때다. [0010](decisions/0010-storage-index-and-korean-search.md)이 색인 크기에 재검토 조건을 단 방식과 같다.

**임베딩이 붙으면 색인의 성질이 바뀐다.** 지금은 색인이 낡으면 호출부가 조용히 메모리에서 다시 만든다(`search`, `mcp`, `serve` 셋). 임베딩이 붙으면 그 자리에서 다시 만들 수 없다. **낡은 색인을 만났을 때 무엇을 할지가 새로 정할 것이다.**

**승급 파이프라인은 바뀌지 않는다.** upstream이 RAG 색인 대상을 승인된 `context/`로 한정하므로 파이프라인이 곧 색인 자격 판정이다. 시맨틱이 들어와도 그 관계는 그대로다.
...
- ~~**규칙 명세와 구현의 대응표.**~~ `docs/spec-map.md`로 만들었다. 명세 7종, lint 규칙 20종, 설정 속성 14종의 대응이 전부 들어 있다.
...
- ~~**`engram model`이 관리할 모델의 범위.**~~ [0068](decisions/0068-model-command-manages-embeddings-only.md)이 임베딩 하나로 정했다. STT는 파일이 만들어지기 전이고 TTS는 꺼낸 다음이라 engram이 볼 자리가 아니다. 사용자가 자기 도구로 준비하고 engram은 결과 텍스트를 받는다.
- 강의 일정, 대상, 평가 방식.
