...
**저장을 SQLite로 옮기는 것을 함께 본다.** 조각마다 벡터를 담으려면 JSON 파일 하나로는 감당이 안 된다. FTS5를 쓰면 토크나이저를 넘길 수도 있으나 **bigram이 이미 조사 문제를 풀고 있으므로 그것이 이유가 되지는 못한다**(0061에 실측이 있다). SQLite의 이유는 벡터 저장 하나다.

**옮길 때 색인의 성질이 바뀐다.** 지금은 색인이 낡으면 호출부가 조용히 메모리에서 다시 만든다(`search`, `mcp`, `serve` 셋). 임베딩이 붙으면 그 자리에서 다시 만들 수 없다. **낡은 색인을 만났을 때 무엇을 할지가 새로 정할 것이다.**
