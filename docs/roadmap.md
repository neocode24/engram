...
**0.3 마일스톤의 커맨드가 전부 동작한다.** 커맨드 스물, ADR 28건, 동작 구조 도식 10종, 여정 24개가 문서로 있고, 코드는 패키지 16개다. Windows 실환경 2차 검증까지 마쳤으나 콘솔 직결 항목은 검증 스크립트 결함으로 아직 미확인이다.
...
**도구가 자기 산출물로 자기 검사를 통과한다.** `init`, `capture`, `source`, `promote`, `new`, `archive`를 순서대로 돌린 위키에서 `lint`의 `error`와 `warn`이 0이다. 구현 중 네 번 이 원칙을 어겼고 네 번 다 고쳤다. 앞의 셋이 ADR 0019, 0021, 0023이고 넷째가 `archive`의 `artifact_stage` 허용값 누락이다.
...
1. **upstream parity 대조** — 지금 harness가 덮는 것은 자기 출력의 골든 스냅샷뿐이다. ADR 0005가 정한 upstream 스크립트와의 실제 대조는 아직 없다. `docs/parity.md`는 그때 생긴다. **이 작업은 upstream 문서를 픽스처로 끌어와야 하므로 공개 경계 설계가 선행한다.** 픽스처를 `private/`에 두는 방식이 유력하며, 그 전까지 외부 프로바이더에 위임하지 않는다.
...
3. **`config` 스키마 upstream 대조** — `ArtifactStages` 허용값에 `archive`가 없어 `archive` 커맨드의 산출물을 `lint`가 거절했다. 이번에 넣었으나 이 집합은 upstream 계약에서 온 것이라 parity 대조 때 확인이 필요하다.
...
- `updated` 필드를 아무도 채우지 않는다. `wiki.Frontmatter`는 이 필드를 git 이력에서 채울 것으로 두었으나 그 `sync`가 아직 없다. `resurface`의 기준 날짜는 `created`로 대체되고 있다.
