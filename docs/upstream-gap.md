<!-- engram:record as-of=2026-08-18 -->
...
**upstream `llm-wiki`의 2026-08-18 상태를 기준으로 읽었다. 그 뒤의 upstream 변경은 반영되어 있지 않다.**

이 표기가 필요한 이유가 있다. 대조를 마친 다음 날 upstream의 `context/procedures/`와 `context/systems/`와 `mirror/README.md`가 바뀌었고, 이 문서가 근거로 삼은 항목 넷의 원문이 함께 바뀌었다. **아무도 그 사실을 알려 주지 않았다.** `scripts/upstream-sync.py`가 그 범위를 안 보기 때문이다. 사정은 [spec-map](spec-map.md) 6.2절에 있다.

그러므로 이 문서의 각 항목을 실제로 만들기 전에 **그 항목이 인용한 upstream 파일을 다시 열어 봐야 한다.** 인용에 `파일:줄`이 붙어 있는 것은 그 확인을 싸게 하기 위해서다.

원본 감사 보고서 다섯은 `private/upstream-audit-2026-08-18/`에 있고 gitignore 대상이라 커밋되지 않는다. 실체는 upstream `meta/engram/upstream-audit-2026-08-18/`에 백업했다([ADR 0033](decisions/0033-private-backup-and-fail-closed-boundary.md)).
...
**풀링 방식을 잘못 잡았던 것을 구현 중에 찾아 고쳤다.** bge-m3 는 CLS 풀링을 쓴다. `BAAI/bge-m3` 의 `1_Pooling/config.json` 이 `pooling_mode_cls_token` 을 참으로 둔다. 그런데 `hugot` 의 특징추출 파이프라인은 출력이 토큰 임베딩이면 평균 풀링을 강제하며 선택지를 주지 않는다.

같은 80문서에서 잰 결과다.

| 하한 | upstream (PyTorch CLS) | engram (ONNX CLS) | engram (평균 풀링) |
|---|---|---|---|
| 0.65 | 145 | 147 | |
| 0.70 | 68 | 59 | |
| 0.72 | **45** | **45** | **2897** |
...
**이 대조는 engram 의 순수 Go ONNX 파이프라인이 upstream 의 PyTorch MPS 파이프라인을 재현한다는 것을 보인다.** 동등성 검증이 `lint` 에 이어 두 번째 축을 얻었다.
...
**주의**: 이 항목이 인용한 `inbox-processing-matrix.md`가 2026-08-19에 바뀌었다. 만들기 전에 원문을 다시 확인한다.
...
**주의**: 이 항목이 인용한 `mirror/README.md`가 2026-08-19에 바뀌었다. 만들기 전에 원문을 다시 확인한다.
...
**주의**: 이 항목이 인용한 `obsidian-operating-layer.md`가 2026-08-19에 바뀌었다. 만들기 전에 원문을 다시 확인한다.
