...
> 2026-08-17 재작성. 여정 24개([journeys.md](journeys.md))와 1.0 커맨드 스물여덟이 확정되어 강의 단위를 다시 매핑했다. 초판(2026-08-08)은 여정 다섯 개 시절의 뼈대였다. 강의 자료는 [course/](course/README.md)에 둔다.

## 대상과 형태

대상은 Claude Code 같은 CLI 에이전트를 이미 쓰는 개발자다. 마크다운과 git을 안다고 전제한다. RAG, frontmatter 같은 용어는 짧게 짚고 넘어간다.

단위는 여덟이다. 1단위는 60분 강의이고 나머지는 각 40분에서 60분의 핸즈온이다. 하루 워크숍이면 1단위부터 6단위까지, 이틀이면 8단위까지 간다. 각 단위는 앞 단위가 만든 위키를 이어서 쓴다. 그래서 순서를 바꾸지 않는다.

교육용 위키는 수강생이 1단위 끝에 `engram init`으로 직접 만든다. `examples/education`은 강사가 결과를 미리 보여 줄 때만 쓴다.
...
여정 2(음성)와 4(주간 뉴스)는 바이너리 밖 동선이라 단위에 넣지 않는다. 3단위에서 "전사 결과와 요약만 capture한다"는 한 줄로 다룬다.

각 단위의 마지막은 `engram lint`다. **도구가 만든 위키는 언제 lint를 돌려도 error가 0이어야 한다**는 불변식을 수강생 위키에서 매 단위 확인한다. 어긋나면 그 자리에서 무엇을 손으로 고쳤는지 찾는다.

## 단위별 핵심 문장

강사가 단위마다 한 번씩 소리 내어 말하는 문장이다. 슬라이드 제목이 된다.
...
시험은 없다. 8단위가 끝났을 때 수강생 위키가 다음을 만족하면 이수다.
...
- 2단위부터 8단위의 자료 형식. 1단위처럼 슬라이드와 읽기 모드를 겸한 단일 HTML로 갈지, 실습 문서는 마크다운으로 둘지.
- 7단위에서 쓸 에이전트. Claude Code를 기본으로 두되 다른 CLI 에이전트도 같은 스킬 문서로 붙는지 확인한다.
