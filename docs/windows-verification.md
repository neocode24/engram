...
- [ ] 한글 제목 capture 의 생성 파일명이 한글과 하이픈으로 온전히 보인다.
      깨졌거나 물음표로 바뀌면 발견이다
- [ ] 한글 출력(콘솔 직결) 항목에서 현황, 적체 압력, 다음 행동 글자가
      온전히 보인다. 이것이 ADR 0026 이 동작한다는 증거이며 이 절차의
      핵심 항목이다. 자세한 것은 아래 캡처와 직결의 차이 절을 본다
- [ ] 한글 출력(파이프, 호스트 기본 인코딩) 항목은 949 콘솔에서 깨지는
      것이 정상이다. 받는 쪽 설정 문제이지 engram 결함이 아니다
- [ ] 한글 출력(파이프, UTF-8 호스트) 항목은 온전히 보인다. 위 항목과
      이 항목의 차이가 원인이 호스트에 있음을 증명한다. 여기까지 깨지면
      engram 이 잘못된 바이트를 내는 것이므로 발견이다
...
검증 스크립트의 세 항목이 이 차이를 나란히 보여준다. 콘솔 직결은
온전하고, 호스트 기본 인코딩의 파이프는 깨지고, 호스트를 UTF-8 로
맞춘 파이프는 다시 온전하다. 이 배열이 나오면 정상 동작이다.

**PowerShell 에서 콘솔 직결을 함수로 감싸면 안 된다.** 함수 호출 결과를
변수에 대입하면 함수 안의 네이티브 명령 stdout 까지 함께 캡처되어
자식 stdout 이 파이프가 된다. 콘솔 직결을 검증한다고 쓴 코드가 실제로는
파이프를 검증하게 되며, 이 함정에 2차 검증이 실제로 걸렸다. 호출부에서
`& $EngramPath ...` 로 직접 부르고 `$LASTEXITCODE` 만 따로 읽는다.

## 결과 보고

스크립트 마지막 요약(PASS, FAIL, 확인필요, SKIP 건수)과 위 체크리스트의
