...
--related <슬러그>를 반복해 이 자리에서 채울 수 있습니다.`,
		...
		"cli.promote.sensitivity_blocked":  "민감도가 %s인 문서는 context로 올릴 수 없습니다. 승급 후보는 public-reference와 internal뿐입니다. 문서의 sensitivity 값을 고친 뒤 다시 시도하세요",
		"cli.promote.secret_blocked":       "시크릿으로 보이는 것이 있어 올릴 수 없습니다. 규칙 이름과 줄 번호만 알립니다. 해당 줄에서 시크릿을 지우세요",
		"cli.promote.secret_finding":       "  %s, %d번째 줄",
...
short, repeat --related <slug> to fill them in here.`,
		...
		"cli.promote.sensitivity_blocked":  "A document with sensitivity %s cannot be promoted to context. Only public-reference and internal are promotion candidates. Fix the sensitivity value and retry",
		"cli.promote.secret_blocked":       "Something looks like a secret, so the document cannot be promoted. Only the rule and line number are reported. Remove the secret on that line",
		"cli.promote.secret_finding":       "  %s, line %d",
