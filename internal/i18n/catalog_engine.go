...
func init() {
	Register(LangKO, map[string]string{
		...
		"lint.rule.schema_indexable_stage":    "단계와 indexable 값의 정합. inbox에서 indexable이 false가 아니면 error, source와 context에서 단계 기본값에서 벗어나면 warn(ADR 0071)",
		"lint.rule.schema_deprecated_field":   "deprecated_fields에 적은 폐기 필드가 프론트매터에 있음(ADR 0071)",
		"lint.rule.security_secret":           "시크릿으로 보이는 문자열. PEM 개인키, AWS 액세스 키, GitHub 토큰, Slack 토큰, Bearer 헤더, 홈 경로 여섯 패턴(ADR 0069)",
		...
		"lint.rule.graph_empty_provenance":    "context 문서의 source_refs가 빈 배열(ADR 0073)",
		...
		"lint.violation.security_secret.message":      "시크릿으로 보이는 문자열이 있습니다 (%s). 값은 알리지 않습니다",
		"lint.violation.security_secret.fix":          "해당 줄의 시크릿을 지우거나 자리 표시자로 바꾸세요",
		...
		"lint.violation.indexable_inbox.message":      "inbox 단계 문서의 indexable이 false가 아닙니다",
		"lint.violation.indexable_inbox.fix":          "indexable을 false로 바꾸세요. inbox는 검수 전 단계라 색인 대상이 아닙니다",
		"lint.violation.indexable_source.message":     "source 단계 문서의 indexable이 false가 아닙니다",
		"lint.violation.indexable_source.fix":         "색인에서 빼려면 indexable을 false로 바꾸세요. 승인으로 색인 대상이면 그대로 두세요",
		"lint.violation.indexable_context.message":    "context 단계 문서의 indexable이 true가 아닙니다",
		"lint.violation.indexable_context.fix":        "색인 대상이면 indexable을 true로 바꾸세요. 일부러 제외한 문서면 그대로 두세요",
		"lint.violation.deprecated_field.message":     "폐기한 필드 %s가 문서에 있습니다",
		"lint.violation.deprecated_field.fix":         "값을 어디로 옮길지 정해 옮긴 뒤 %s 필드를 지우세요",
		...
		"lint.violation.empty_provenance.message":     "context 문서의 source_refs가 빈 배열입니다",
		"lint.violation.empty_provenance.fix":         "뒷받침하는 원본을 sources에 두고 그 경로를 source_refs에 넣으세요. inbox의 원본은 promote --to sources로 먼저 옮길 수 있습니다",
...
`,
		...
		"eject.agents_doc.contract_heading": "## 판단 규칙은 meta/agent-contract.md 에 있다\n\n",
		"eject.agents_doc.contract_body":    "meta/ 의 나머지 문서와 린터는 형식을 판정한다. 무엇을 원문 그대로 넣을지, 무엇을 올릴 값어치가 있다고 볼지, 어디까지 혼자 정하고 어디서 사람에게 물을지는 형식이 아니라 판단이며 meta/agent-contract.md 가 규정한다. 이 위키에서 처음 일하는 에이전트는 그 문서를 먼저 읽는다.\n\n",
		"eject.agents_doc.dirs_heading":     "## 디렉토리\n\n",
		"eject.contract_doc.heading":        "# 에이전트 작업 계약\n\n",
		"eject.contract_doc.intro":          "이 문서는 engram 스킬 문서의 본문이며 eject 가 위키 안으로 옮긴 것이다. 판단 규칙이 사용자 홈에만 있으면 이 위키를 복제한 사람에게 닿지 않는다. 형식 판정은 scripts/lint-frontmatter.py 가 하고 이 문서는 판단을 규정한다.\n\n",
...
`,
    ...
    `,
		...
		"eject.linter.indexable_inbox.message":   "inbox 단계 문서의 indexable이 false가 아닙니다",
		"eject.linter.indexable_inbox.fix":       "indexable을 false로 바꾸세요. inbox는 검수 전 단계라 색인 대상이 아닙니다",
		"eject.linter.indexable_source.message":  "source 단계 문서의 indexable이 false가 아닙니다",
		"eject.linter.indexable_source.fix":      "색인에서 빼려면 indexable을 false로 바꾸세요. 승인으로 색인 대상이면 그대로 두세요",
		"eject.linter.indexable_context.message": "context 단계 문서의 indexable이 true가 아닙니다",
		"eject.linter.indexable_context.fix":     "색인 대상이면 indexable을 true로 바꾸세요. 일부러 제외한 문서면 그대로 두세요",
		"eject.linter.deprecated_field.message":  "폐기한 필드 %s가 문서에 있습니다",
		"eject.linter.deprecated_field.fix":      "값을 어디로 옮길지 정해 옮긴 뒤 %s 필드를 지우세요",
		...
		"eject.linter.empty_provenance.message":  "context 문서의 source_refs가 빈 배열입니다",
		"eject.linter.empty_provenance.fix":      "뒷받침하는 원본을 sources에 두고 그 경로를 source_refs에 넣으세요. inbox의 원본은 promote --to sources로 먼저 옮길 수 있습니다",
	...
	Register(LangEN, map[string]string{
		...
		"lint.rule.schema_indexable_stage":    "indexable value versus stage. Non-false in inbox is an error; deviations from the stage defaults in source and context are warns (ADR 0071)",
		"lint.rule.schema_deprecated_field":   "A retired field listed in deprecated_fields is present in the frontmatter (ADR 0071)",
		"lint.rule.security_secret":           "String that looks like a secret: one of six patterns, PEM private key, AWS access key, GitHub token, Slack token, Bearer header, home path (ADR 0069)",
		...
		"lint.rule.graph_empty_provenance":    "context document whose source_refs is an empty array (ADR 0073)",
		...
		"lint.violation.security_secret.message":      "Something looks like a secret (%s). The value itself is not shown",
		"lint.violation.security_secret.fix":          "Remove the secret on that line or replace it with a placeholder",
		...
		"lint.violation.indexable_inbox.message":      "An inbox document has indexable other than false",
		"lint.violation.indexable_inbox.fix":          "Set indexable to false. Inbox is the unreviewed stage and is not for indexing",
		"lint.violation.indexable_source.message":     "A source document has indexable other than false",
		"lint.violation.indexable_source.fix":         "Set indexable to false to keep it out of the index. Leave it as is if it was approved for indexing",
		"lint.violation.indexable_context.message":    "A context document has indexable other than true",
		"lint.violation.indexable_context.fix":        "Set indexable to true if it is an index target. Leave it as is if it was excluded on purpose",
		"lint.violation.deprecated_field.message":     "Retired field %s is present in the document",
		"lint.violation.deprecated_field.fix":         "Decide where the value moves, move it, then remove the %s field",
		...
		"lint.violation.empty_provenance.message":     "A context document has an empty source_refs array",
		"lint.violation.empty_provenance.fix":         "Put the backing original in sources and add its path to source_refs. An inbox original can be moved first with promote --to sources",
...
`,
		...
		"eject.agents_doc.contract_heading": "## The judgment rules are in meta/agent-contract.md\n\n",
		"eject.agents_doc.contract_body":    "The other documents under meta/ and the linter judge form. What to capture verbatim, what is worth promoting, and where to decide alone versus ask a human are judgment rather than form, and meta/agent-contract.md governs them. An agent working in this wiki for the first time reads that document first.\n\n",
		"eject.agents_doc.dirs_heading":     "## Directories\n\n",
		"eject.contract_doc.heading":        "# Agent working contract\n\n",
		"eject.contract_doc.intro":          "This document is the body of the engram skill document, moved into the wiki by eject. Judgment rules that live only in a user's home directory never reach someone who clones this wiki. scripts/lint-frontmatter.py judges form; this document governs judgment. The body below is Korean because the skill document is maintained in one language.\n\n",
...
`,
    ...
    `,
		...
		"eject.linter.indexable_inbox.message":   "An inbox document has indexable other than false",
		"eject.linter.indexable_inbox.fix":       "Set indexable to false. Inbox is the unreviewed stage and is not for indexing",
		"eject.linter.indexable_source.message":  "A source document has indexable other than false",
		"eject.linter.indexable_source.fix":      "Set indexable to false to keep it out of the index. Leave it as is if it was approved for indexing",
		"eject.linter.indexable_context.message": "A context document has indexable other than true",
		"eject.linter.indexable_context.fix":     "Set indexable to true if it is an index target. Leave it as is if it was excluded on purpose",
		"eject.linter.deprecated_field.message":  "Retired field %s is present in the document",
		"eject.linter.deprecated_field.fix":      "Decide where the value moves, move it, then remove the %s field",
		...
		"eject.linter.empty_provenance.message":  "A context document has an empty source_refs array",
		"eject.linter.empty_provenance.fix":      "Put the backing original in sources and add its path to source_refs. An inbox original can be moved first with promote --to sources",
