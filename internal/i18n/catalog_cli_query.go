...
어느 쪽이든 inbox를 포함합니다.`,
		...
		"cli.lint.summary_clean_inbox_skipped": "검사한 파일 %d개, inbox 문서 %d개는 스키마 판정에서 뺐습니다(링크는 봅니다. 전부 보려면 --include-inbox), 위반 없음",
		"cli.lint.summary_inbox_skipped":       "검사한 파일 %d개, inbox 문서 %d개는 스키마 판정에서 뺐습니다(링크는 봅니다. 전부 보려면 --include-inbox), error %d, warn %d, reject %d",
...
낡은 색인은 경고를 내고 그대로 진행합니다.`,
		...
		"cli.bridge.embed_start":        "문서 %d개의 벡터를 만듭니다. 모델을 적재하는 데 1분쯤 걸리고 그동안 숫자가 움직이지 않습니다",
...
either way.`,
		...
		"cli.lint.summary_clean_inbox_skipped": "Files checked: %d, %d inbox documents left out of the schema rules (links are still checked; --include-inbox checks everything), no violations",
		"cli.lint.summary_inbox_skipped":       "Files checked: %d, %d inbox documents left out of the schema rules (links are still checked; --include-inbox checks everything), error %d, warn %d, reject %d",
...
A stale index warns and proceeds as is.`,
		...
		"cli.bridge.embed_start":        "Building vectors for %d documents. Loading the model takes about a minute and the counter will not move during it",
